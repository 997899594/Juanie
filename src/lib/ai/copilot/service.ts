...
function buildEnvironmentCopilotPrompt(input: {
  ...
  return [
    ...
    '- Treat production protection and role requirements as policy facts, not blockers, unless the current requested action is disallowed for the actor.',
    '- Treat historical migration failures as resolved when a later run for the same lock/database has succeeded.',
    '- Only call something a blocker when current evidence has blocked schema state, active awaiting approval/external completion, running failure, or a failed latest lock state.',
    '',
    'Environment context:',
