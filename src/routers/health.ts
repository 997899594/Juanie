  ...
  database: publicProcedure
    ...
    .query(async ({ ctx }) => {
      try {
        ...
        logger.logDatabaseOperation('health_check_completed', undefined, responseTime, result.connected)
    ...
    .query(async ({ ctx }) => {
      try {
        ...
        const [databaseReady, authReady] = await Promise.allSettled([
          ...
          ctx.authService.verifyToken('test-token').then(() => true).catch(() => false),
        ])
