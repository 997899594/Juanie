...
IMAGE_PULL_SECRET_NAME="${DEPLOY_REGISTRY_PULL_SECRET_NAME:-ghcr-pull-secret}"
...
ssh ${SSH_OPTS} "${SERVER_USER:?SERVER_USER is required}@${SERVER_HOST:?SERVER_HOST is required}" \
  "WEB_IMAGE='${WEB_IMAGE}' WORKER_IMAGE='${WORKER_IMAGE}' MIGRATE_IMAGE='${MIGRATE_IMAGE}' IMAGE_REPOSITORY='${IMAGE_REPOSITORY}' WEB_IMAGE_TAG='${WEB_IMAGE_TAG}' WORKER_IMAGE_TAG='${WORKER_IMAGE_TAG}' MIGRATE_IMAGE_TAG='${MIGRATE_IMAGE_TAG}' IMAGE_PULL_SECRET_NAME='${IMAGE_PULL_SECRET_NAME}' CHART_DIR='${CHART_DIR}' bash -s" \
