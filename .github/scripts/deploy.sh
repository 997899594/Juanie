...
SCHEMA_RUNNER_IMAGE_TAG="schema-runner-${GITHUB_SHA}"
...
retry 6 ssh "${SSH_OPTS[@]}" "${SERVER_USER:?SERVER_USER is required}@${SERVER_HOST:?SERVER_HOST is required}" \
  "IMAGE_REGISTRY_B64='$(encode_env "${IMAGE_REGISTRY}")' FULL_IMAGE_REPOSITORY_B64='$(encode_env "${FULL_IMAGE_REPOSITORY}")' WEB_IMAGE_TAG_B64='$(encode_env "${WEB_IMAGE_TAG}")' WORKER_IMAGE_TAG_B64='$(encode_env "${WORKER_IMAGE_TAG}")' SCHEMA_RUNNER_IMAGE_TAG_B64='$(encode_env "${SCHEMA_RUNNER_IMAGE_TAG}")' REMOTE_DIR_B64='$(encode_env "${REMOTE_DIR}")' bash '${REMOTE_DEPLOY_SCRIPT}'"
