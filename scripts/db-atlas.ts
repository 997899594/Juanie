...
const ATLAS_DOCKER_IMAGE = process.env.ATLAS_DOCKER_IMAGE ?? `arigaio/atlas:${ATLAS_VERSION}-community`;
...
async function getAtlasRevisionCount(databaseUrl: string): Promise<number> {
  ...
  try {
    ...
    const rows = await sql.unsafe(`select count(*)::int as count from "${REVISIONS_SCHEMA}"."${ATLAS_REVISIONS_TABLE}"`);
...
async function hashMigrations(): Promise<void> {
...
async function withDockerDevDatabase<T>(task: (input: {
  devUrl: string;
  networkName: string;
}) => Promise<T>): Promise<T> {
  if (!hasExecutable('docker')) {
...
async function generateMigration(name: string | undefined): Promise<void> {
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'apply':
      ...
      return;
    default:
      throw new Error('Usage: bun scripts/db-atlas.ts <generate|hash|validate|status|apply> [name]');
  }
}
