...
export class AuthService {
  ...
  constructor(
    ...
    try {
      ...
      const gitlabRedirectUri = this.configService.get<string>('GITLAB_REDIRECT_URI')
      const gitlabBaseUrl = this.configService.get<string>('GITLAB_BASE_URL')
      ...
      const hasValidGitLabConfig =
        ...
        gitlabClientSecret &&
        gitlabRedirectUri &&
        gitlabClientId !== 'your_gitlab_client_id'

      if (hasValidGitHubConfig) {
        this.github = new GitHub(githubClientId, githubClientSecret, githubRedirectUri)
