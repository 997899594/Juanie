...
export class GitRepositoryService {
  ...
  async connectRepository(
    ...
    try {
      ...
      const [existingRepo] = await this.db.drizzle
        ...
        .where(
          and(
            eq(gitRepositories.repoId, repoInfo.repoId),
            eq(gitRepositories.provider, provider)
          )
        )
        .limit(1);
  ...
  async getRepositories(
    ...
    const repositories = await query
      .limit(limit)
      .offset(offset)
      .orderBy(gitRepositories.createdAt);
  ...
  async syncRepository(repositoryId: string): Promise<void> {
    ...
    try {
      const gitProvider = this.gitProviderFactory.create(
        repo.provider,
        repo.accessToken!
      );
