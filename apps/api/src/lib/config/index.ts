...
const configSchema = z
  ...
  .transform((data) => ({
    ...
    canUseRateLimit:
      data.rateLimit.enabled && Boolean(data.redis.url && data.redis.token),
    canUseCache:
      data.cache.enabled && Boolean(data.redis.url && data.redis.token),
  }));
...
export function validateConfig(config: Config): void {
  ...
  if (config.cache.enabled && !config.hasRedis) {
    console.warn(
      "⚠️  Caching is enabled but Redis is not configured. Caching will be disabled."
    );
  }
  ...
  if (
    ...
    console.warn(
      "⚠️  Tracing is enabled but no endpoint configured in production."
    );
  }
