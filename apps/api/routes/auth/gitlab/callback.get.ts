import { createError, defineEventHandler, getCookie, getQuery, sendRedirect, setCookie } from 'h3'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const user = await authService.validateGitLabCallback(code as string, codeVerifier)
