...
Juanie persists one specification per architecture stage and records an immutable execution
snapshot on every migration run. Stage order, target version, source path, policies, and execution
tool therefore cannot change underneath a running release when a newer commit is synchronized.
The release phase scheduler orders runs by stage order, not insertion timestamp.
...
- The control-plane schema gains stage metadata and immutable run snapshots.
...
- Retry creates a new run from the immutable stage snapshot and the same release commit.
