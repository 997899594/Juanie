...
import type { GetCostSummaryInput, ListCostsInput, RecordCostInput } from '@juanie/core-types'
...
export class CostTrackingService {
  ...
  // 记录成本数据
  @Trace('cost-tracking.record')
  async record(userId: string, data: RecordCostInput) {
    // 检查权限
  ...
  // 列出成本记录
  @Trace('cost-tracking.list')
  async list(userId: string, filters: ListCostsInput) {
  ...
  // 获取成本汇总
  @Trace('cost-tracking.getSummary')
  async getSummary(userId: string, filters: GetCostSummaryInput) {
    const hasAccess = await this.checkOrgAccess(userId, filters.organizationId)
    if (!hasAccess) {
      throw new Error('没有权限访问该组织')
    }
