 ...
 */
export class QwenAdapter implements IAIClient {
  private provider: ReturnType<typeof createOpenAI>
  private model: ReturnType<ReturnType<typeof createOpenAI>>

  constructor(private readonly config: AIClientConfig) {
    if (!config.apiKey) {
      throw ErrorFactory.ai.inferenceFailed('Qwen API key is required')
    }

    // 阿里云 Qwen 使用 OpenAI 兼容的 API
  ...
  async complete(options: AICompletionOptions): Promise<AICompletionResult> {
    try {
      ...
      throw ErrorFactory.ai.inferenceFailed(
        `Qwen API error: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
    }
  }
  ...
  async *streamComplete(options: AICompletionOptions): AsyncIterable<string> {
    try {
      ...
      throw ErrorFactory.ai.inferenceFailed(
        `Qwen streaming error: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
    }
  }
