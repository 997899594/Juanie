...
export class WebhookEventProcessor {
  ...
  private async processGitHubRepositoryEvent(payload: any): Promise<void> {
    ...
    // ✅ 发布内部事件 - 使用 DomainEvents 和 emit()
    this.eventEmitter.emit(DomainEvents.GIT_REPOSITORY_CHANGED, {
      provider: 'github',
      action,
      repository: {
  ...
  private async processGitHubCollaboratorEvent(payload: any): Promise<void> {
    ...
    // ✅ 发布内部事件 - 使用 DomainEvents 和 emit()
    this.eventEmitter.emit(DomainEvents.GIT_COLLABORATOR_CHANGED, {
      provider: 'github',
      action,
      repository: {
        gitId: repository.id.toString(),
  ...
  private async processGitHubMemberEvent(payload: any): Promise<void> {
    ...
    // ✅ 发布内部事件 - 使用 DomainEvents 和 emit()
    this.eventEmitter.emit(DomainEvents.GIT_MEMBER_CHANGED, {
      provider: 'github',
      action,
      organization: {
  ...
  private async processGitHubOrganizationEvent(payload: any): Promise<void> {
    ...
    // ✅ 发布内部事件 - 使用 DomainEvents 和 emit()
    this.eventEmitter.emit(DomainEvents.GIT_ORGANIZATION_CHANGED, {
      provider: 'github',
      action,
      organization: {
        gitId: organization.id.toString(),
  ...
  private async processGitHubPushEvent(payload: any): Promise<void> {
    ...
    // ✅ 发布内部事件 - 使用 DomainEvents 和 emit()
    this.eventEmitter.emit(DomainEvents.GIT_PUSH, {
      provider: 'github',
      repository: {
        gitId: repository.id.toString(),
        name: repository.name,
