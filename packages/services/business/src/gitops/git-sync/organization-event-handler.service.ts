...
export class OrganizationEventHandler {
  ...
  async handleOrganizationCreated(event: OrganizationCreatedEvent) {
    ...
    try {
      ...
      this.logger.log(`Queued Git organization creation for: ${event.name}`)
    } catch (error) {
  ...
  async handleMemberAdded(event: OrganizationMemberAddedEvent) {
    try {
      ...
      this.logger.log(`Queued member addition sync for user: ${event.userId}`)
    } catch (error) {
  ...
  async handleMemberRemoved(event: OrganizationMemberRemovedEvent) {
    try {
      ...
      this.logger.log(`Queued member removal sync for user: ${event.userId}`)
    } catch (error) {
