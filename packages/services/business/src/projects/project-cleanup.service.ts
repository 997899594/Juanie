...
export class ProjectCleanupService {
    ...
    try {
      ...
      for (const project of failedProjects) {
        try {
          ...
          // 清理 GitOps 资源
          const result = await this.fluxResources.cleanupProjectGitOps(project.id)

          if (result.success) {
            this.logger.info(
              `Successfully cleaned up GitOps resources for project ${project.id}: ${result.deletedResources.join(', ')}`,
            )
          } else {
            this.logger.warn(
              `Failed to cleanup some resources for project ${project.id}: ${result.errors.join(', ')}`,
            )
          }
    ...
    try {
      ...
      for (const project of testProjects) {
        try {
          ...
          // 清理 GitOps 资源
          await this.fluxResources.cleanupProjectGitOps(project.id)
  ...
  async cleanupProject(projectId: string): Promise<{
    ...
    try {
      ...
      // 清理 GitOps 资源
      const result = await this.fluxResources.cleanupProjectGitOps(projectId)

      if (!result.success) {
        return {
          success: false,
          message: `Failed to cleanup some resources: ${result.errors.join(', ')}`,
        }
      }
