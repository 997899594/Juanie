...
export class ProjectMembersService {
  ...
  async addMember(
    ...
    // ✅ 触发 Git 平台权限同步事件 - 使用 EventEmitter2.emit()
  ...
  async updateMemberRole(
    ...
    // ✅ 触发 Git 平台权限更新事件 - 使用 EventEmitter2.emit()
    // Requirements: 4.7 - 使用事件驱动架构解耦
    this.eventEmitter.emit(DomainEvents.PROJECT_MEMBER_UPDATED, {
      projectId,
      userId,
      memberId: data.userId,
      role: this.mapRoleToProjectRole(data.role),
  ...
  async removeMember(userId: string, projectId: string, data: { userId: string }) {
    ...
    // ✅ 触发 Git 平台权限移除事件 - 使用 EventEmitter2.emit()
    // Requirements: 4.8 - 使用事件驱动架构解耦
    this.eventEmitter.emit(DomainEvents.PROJECT_MEMBER_REMOVED, {
      projectId,
      userId,
      memberId: data.userId,
      timestamp: new Date(),
