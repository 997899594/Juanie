<template>
  <div class="edit-project-form">
    ...
    >
      ...
      <n-form-item path="status" label="项目状态">
        <n-select
      ...
      <div class="stats-section">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-item">
            <div class="stat-value">{{ project.deploymentCount || 0 }}</div>
            <div class="stat-label">部署次数</div>
          </div>
          <div class="stat-item">
            <div class="stat-value">{{ project.pipelineCount || 0 }}</div>
            <div class="stat-label">流水线</div>
          </div>
          <div class="stat-item">
            <div class="stat-value">{{ formatDate(project.createdAt) }}</div>
...
const isFormValid = computed(() => {
  return formData.value.name && formData.value.defaultBranch && formData.value.status && formData.value.environment
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const updatedProject = await updateProject(props.project.id, projectData)
...
onMounted(() => {
  ...
  formData.value = {
    ...
    autoDeployment: false, // 这些设置需要从API获取
    codeQualityCheck: true,
    securityScan: true,
    notifications: ['email']
  }
